...
# Clean — remove blank rows and section header rows
df = df[df["Name"].notna()]
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .section-line {{ flex: 1; height: 1px; background: var(--border); margin-top: 8px; min-width: 20px; max-width: 60px; }}
  ...
  @media (max-width: 600px) {{
    ...
    .modal-hero, .modal-body {{ padding: 20px; }}
  }}
</style>
</head>
<body>
...
// Build category sidebar dynamically from data
const catCounts = {{}};
...
function buildCard(d) {{
  ...
  el.innerHTML = `
    ...
    <div class="card-bottom">
      <span class="card-funding">${{funding}}</span>
      <span class="card-employees">👥 ${{emp}}</span>
    </div>`;
  return el;
}}
...
function openModal(d) {{
  ...
  const inv = d.investors || null;
  document.getElementById('modal-content').innerHTML = `
    <div class="modal-hero">
      <div class="modal-header-row">
        <div class="modal-avatar">${{initials(d.name)}}</div>
        <div><div class="modal-name">${{d.name}}</div><div class="modal-hq">📍 ${{d.hq || 'Africa'}}</div></div>
      </div>
    ...
    <div class="modal-body">
      <div class="modal-grid">
        ...
        <div class="modal-field"><label>CEO / Founder</label><p>${{d.ceo || '—'}}</p></div>
      </div>
      ${{inv ? `<div class="modal-sub">Investors & Partners</div><div class="modal-investors">${{inv}}</div>` : ''}}
...
output_file = "africa-crypto-directory.html"
with open(output_file, "w", encoding="utf-8") as f:
    f.write(html)

print(f"\n✅ HTML saved to: {output_file}")
print("   Open it in your browser to view the directory.")